...
JAVA_OPTS=" -Dabc.instrument.array.operations -Dabc.instrument.fields.operations -Dabc.taint.android.intents -Dabc.instrument.include=org.liberty.android.fantastischmemo"
...
if [ "$$IS_RUNNING" == "0" ]; then \
        ...
        APP_INSTALLED=$$($(ADB) shell pm list packages | grep -c "org.liberty.android.fantastischmemodev$$"); \
        INSTRUMENTATION_TESTS_INSTALLED=$$($(ADB) shell pm list packages | grep -c "org.liberty.android.fantastischmemodev.test$$"); \
        if [ "$$APP_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing app"); \
                $(ADB) uninstall org.liberty.android.fantastischmemodev > /dev/null; \
        else \
                (>&2 echo "App not installed"); \
        fi; \
        if [ "$$INSTRUMENTATION_TESTS_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing test app"); \
                $(ADB) uninstall org.liberty.android.fantastischmemodev.test > /dev/null; \
        else \
...
$(shell if [ "$(TRACING_APKS_INSTALLED)" == "0" ]; then \
	...
	$(ADB) shell pm clear org.liberty.android.fantastischmemodev > /dev/null; \
fi)
...
app-original.apk : 
	...
	$(GW) -PjacocoEnabled=false assembleDevApi23 && \
	mv app/build/outputs/apk/devApi23/debug/AnyMemo-dev-api23-debug.apk app-debug.apk && \
...
app-original-for-coverage.apk app-androidTest-for-coverage.apk:
	@export ABC_CONFIG=$(ABC_CFG) && \
	$(GW) -PjacocoEnabled=true clean assembleDevApi23 assembleAndroidTest && \
	mv app/build/outputs/apk/devApi23/debug/AnyMemo-dev-api23-debug.apk app-original-for-coverage.apk  && \
	mv app/build/outputs/apk/androidTest/devApi23/debug/AnyMemo-dev-api23-debug-androidTest.apk app-androidTest-for-coverage.apk
